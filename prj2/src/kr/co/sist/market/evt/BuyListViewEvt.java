...
public class BuyListViewEvt extends MouseAdapter implements ActionListener {
	...
	public void setBuyListComp() {
		try {
			...
			for (int i = 0; i < lstItem.size(); i++) {
				...
				sbv = lstItem.get(i); //리스트메뉴의 값을 가져와서 SellBuyVO에 저장
				rowItem[0] = (i + 1); //구매번호
				rowItem[1] = sbv.getId(); //판매자ID
				rowItem[2] = sbv.getItemCode(); //제품코드
				rowItem[3] = sbv.getItemName(); //제품명
				rowItem[4] = sbv.getTradeDate(); //구매완료일시
				dtmItem.addRow(rowItem);
			} // end for
		} catch (SQLException e) {
			JOptionPane.showMessageDialog(blv, "죄송합니다. 구매완료목록을 불러올 수 없습니다.\n잠시후 다시 시도해주세요.");
			e.printStackTrace();
		}
	}// setSellList()
	...
	public void setBuyListWait() {
		try {
			...
			Object[] rowItem = new Object[5];
			// DefaultTableModel을 받아와야한다.
			DefaultTableModel dtmItem = blv.getDtmWait(); //BuyListView클래스에서dtm객체를 가져옴
			SellBuyVO sbv = null;
			for (int i = 0; i < lstItem.size(); i++) {
				// SellBuyVO //String id, itemCode, itemName, tradeDate;
				sbv = lstItem.get(i); // 리스트메뉴의 값을 가져와서 sbv에 집어넣음
				rowItem[0] = (i + 1); //번호
				rowItem[1] = sbv.getId(); //판매등록자ID
				rowItem[2] = sbv.getItemCode(); //상품코드
				rowItem[3] = sbv.getItemName(); //상품명
				rowItem[4] = sbv.getTradeDate(); //신청일
				dtmItem.addRow(rowItem);
			} // end for
		} catch (SQLException e) {
			JOptionPane.showMessageDialog(blv, "죄송합니다. 구매완료목록을 불러올 수 없습니다.\n잠시후 다시 시도해주세요.");
